...
func (rcs *ContentResolver) CreatedBy(ctx context.Context, obj *pb.Content) (*user.User, error) {
	...
	if err != nil {
		errorutils.AddGQLError(ctx, fmt.Errorf("unable to retreieve user %s",err))
		rcs.Logger.Error(err)
		return nil, err
	}
	userId,err:=strconv.ParseInt(userResp.ID,10,64)
	if err != nil {
		errorutils.AddGQLError(ctx, fmt.Errorf("unable to convert user id to integer %s",err))
		rcs.Logger.Error(err)
		return nil, err
	}
	rcs.Logger.Debugf("successfully found user with id %d", email)
	return &user.User{
		Data: &user.UserData{
			Type: "user",
			Id:   userId,
